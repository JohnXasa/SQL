...
# - employee number
# - the smallest department number among the departments where an employee has worked in (use a subquery to retrieve this value from 
# the 'dept_emp' table)
# - assign '110022' as 'manager' to all individuals whose employee number is less than or equal to 10020, and '110039' to those whose 
# number is between 10021 and 10040 inclusive (use a CASE statement to create the third field).
...
# After LIKE, you could proceed to indicate what you are looking for with or without using parentheses. Both options work correctly.
# We think using parentheses is better for legibility reasons and that’s why it is the first option we’ve suggested.
SELECT
    *
FROM
    titles
...
delimiter $$
Create function f_largest_salary(p_emp_no int) returns decimal(10,2)
deterministic
begin
